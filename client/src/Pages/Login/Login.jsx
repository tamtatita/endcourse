...
const Login = () => {
  ...
  const onFinishLoginEmail = async (values) => {
    console.log(values);
    const dataSubmit = {
      role: 'teacher',
      email: values.email,
    ...
    try {
      const res = await request.post("action/login", dataSubmit);
      console.log(res);
      if (res.status === 500) {
        message.error("Sai thông tin đăng nhập, hãy kiểm tra lại");
      } else {
        message.success("Đã đăng nhập thành công");
        const user = res.data;
        console.log(user[0].teacherID, "user login");
        let role = "";
        if (user[0].teacherID) {
          role = "teacher";
        } else {
      ...
      <div class="w-full bg-gray-100 lg:w-1/2 flex items-center justify-center">
        <div class="max-w-md w-full p-6">
          ...
          <h1 class="text-sm font-semibold mb-6 text-gray-500 text-center">
            Giảng viên cần đăng nhập vào hệ thống để sử dụng{" "}
          </h1>
          ...
          >
            ...
            <Form.Item>
              <button
                className="w-full bg-blue-600 text-white p-2 rounded-md hover:bg-gray-800 focus:outline-none focus:bg-blue-500  focus:ring-2 focus:ring-offset-2 focus:ring-blue-900 transition-colors duration-300"
                type="primary"
                htmlType="submit"
              >
                Đăng nhập
