...
const ModalAddFileInClass = () => {
  ...
  const onFinish = async (values) => {
    ...
    if (selectFile == "" && currentUser !== null) {
      ...
      const fileRef = ref(storage, `document/${currentUser[0].teacherID}`);
      ...
      if (selectFile) {
        await uploadString(fileRef, selectFile, "data_url").then(async () => {
          ...
          const res = await request.post(`document/add`, dataSubmit);
          console.log(res, "sau khi res");
          if (res.status === 201) {
            message.success("Đã tạo bài giảng cho các lớp thành công");
          } else {
            message.error("Lỗi xảy ra khi tạo bài giảng");
          }
        });
      }
    }
    ...
    <div className="">
      ...
      >
          ...
          <div className="">
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: "Hãy nhập tên tiêu đề bài giảng!",
                },
              ]}
            >
              <Input />
        ...
        <div className="w-1/2">
          ...
          <Form.Item>
            <button
              className="ICON bg-purple-600 text-white w-full"
              type="primary"
              htmlType="submit"
            >
              <BsFillSendFill />
...
export default ModalAddFileInClass;
